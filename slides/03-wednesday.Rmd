...
.f3[We don't need curly braces for functions with only one statement]
...
```{r, eval=FALSE}
f <- function(x)  y <- (x*0.2885) y # WILL THROW ERROR  
```
...
.f3[We can specify .green.b.code[.f] as:
- an existing function  
]
...
.f3[Cleaning text can be **time-consuming** and the first hurdle to address when working with less-than-perfect data.]
...
.f3[✋ How can we tell if the regular expression is matching anything?]
...
.f3[The power and flexibility of regular expressions comes from their ability to describe more complex patterns.]
